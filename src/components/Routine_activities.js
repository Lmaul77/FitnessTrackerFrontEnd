      ...
      {routines.map((routine, index) => {
        return (
          <div className="testing">
            <div
              className="Routine_Activities_ROUTINE"
              key={`allRoutines: ${index}`}
            >
              <h2 className="RoutineActivitiesName">Name: {routine.name}</h2>
              <h3>Goal: {routine.goal}</h3>
              <h4 className="RoutineActivitiesCreator">
            ...
            <div className="Routine_Activities_ACTIVITY">
              {routine.activities.map((activity, index) => (
                <div
                  className="SingeActivitytest"
                  key={`allRoutinesActs: ${activity.id}`}
                >
